<?php
    ...
    abstract class Item //абстрактный суперкласс "Предмет"
    {
        protected $name;
    ...
    class CityVehicle extends Product
    {
        const WHEEL_COUNT = 4;
    ...
    class Car extends CityVehicle implements Paintable, Colorable
    {
        ...
        public function __construct(
            ...
            $discount = 0)
        {
            parent::__construct($name, $description, $price, $category, $discount);
    ...
    class Gadget extends Product implements Powerable
    {
    ...
    class WritingItem extends Product implements Colorable
    {
        protected $color;
...
//==== Игрушки и утка ====
    ...
    class Toy extends Product implements Pressable
    {
        protected $color;
        protected $weight;
    ...
    class Duck extends Toy implements Pressable
    {
        ...
        public function __construct(
            ...
            $discount = 0)
        {
            parent::__construct($name, $weight, $description, $category, $discount);
    ...
    $silverCar = new Car('Opel', [255,255,0], 2300, 560000, '', 'Cars');
    $siennaCar = new Car('Mazda',[160, 82, 45], 2200, 680000, '', 'Cars');
    
    $silverCar->setMaxSpeed(300);
    echo $silverCar->getMaxSpeed().PHP_EOL;
    
    $tvSet1 = new TvSet('TV00001','Samsung',15,23000, 'Популярная модель', 'TV');
    $tvSet2 = new TvSet('TV00002', 'LG',21, 21000, 'Большой телевизор', 'TV');
    ...
    $bluePen = new Pen('Paccard',[0, 0, 255], '230');
    $greenPen = new Pen('Rubin',[0, 255, 0], 327);
    
    print_r($bluePen->getColor());
    
    $standardYellowDuck = new Duck('Yellow Rubber Duck',[255, 255, 0], 20, 40);
    $hugeCyanDuck = new Duck('Cyan Duck',[0, 255, 255], 30, 45);
    
    echo $hugeCyanDuck->getWeight().PHP_EOL;
    $hugeCyanDuck->press();
    
    $huawei = new Product('Huawei P20 Lite', 174, 19999, 'Самый доступный смартфон.', 'Смартфоны', 0);
    $ps4Pro = new Product('Playstation 4 Pro', 330, 29999, 'Самая ожидаемая новинка.', 'Игровые приставки', 10);
